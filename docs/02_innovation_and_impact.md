...
| **R9** | Bug conocido en `get_timeseries_metrics` | En `accessor.py:460` el RMSE se computa contra el cubo completo `ds`, no contra la ventana `sample`; los fallos se tragan en un `warnings.warn` (`:473-474`), dejando `metrics_dict["rmse"]` con longitud distinta al resto en el `xr.concat` de `:501` | Corregir antes de usar métricas por ventana móvil, o evitar la vía RMSE en esa función |
